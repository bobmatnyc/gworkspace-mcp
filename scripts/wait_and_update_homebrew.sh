#!/bin/bash
# Wait for PyPI package availability and update Homebrew formula for gworkspace-mcp
...
check_pypi_availability() {
    local version=$1
    local url="https://pypi.org/pypi/gworkspace-mcp/$version/json"
...
while [ $attempt -le $MAX_ATTEMPTS ]; do
    ...
    if check_pypi_availability "$VERSION"; then
        ...
        if [ -z "$HOMEBREW_TAP_TOKEN" ]; then
            ...
            echo "   python3 scripts/update_homebrew.py --version $VERSION --verbose"
            echo ""
            echo "3. Or update manually:"
            echo "   cd /path/to/homebrew-gworkspace-mcp"
            echo "   # Edit Formula/gworkspace-mcp.rb"
            echo "   # Update version and sha256"
            echo "   git add Formula/gworkspace-mcp.rb"
        ...
        if python3 scripts/update_homebrew.py --version "$VERSION" --verbose; then
            echo ""
            echo "=========================================="
...
echo "2. Check availability: https://pypi.org/project/gworkspace-mcp/$VERSION/"
...
echo "   cd \$(mktemp -d)"
echo "   git clone https://github.com/masapasa/homebrew-gworkspace-mcp.git"
echo "   cd homebrew-gworkspace-mcp"
echo ""
echo "   # Get SHA256 from PyPI"
echo "   PYPI_SHA256=\$(curl -s https://pypi.org/pypi/gworkspace-mcp/$VERSION/json | python3 -c \"import sys, json; data = json.load(sys.stdin); sdist = [r for r in data['urls'] if r['packagetype'] == 'sdist'][0]; print(sdist['digests']['sha256'])\")"
echo ""
echo "   # Update formula"
echo "   sed -i '' \"s/version \\\"[^\\\"]*\\\"/version \\\"$VERSION\\\"/g\" Formula/gworkspace-mcp.rb"
echo "   sed -i '' \"s/sha256 \\\"[^\\\"]*\\\"/sha256 \\\"\$PYPI_SHA256\\\"/g\" Formula/gworkspace-mcp.rb"
echo ""
