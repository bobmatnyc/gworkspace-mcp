#!/usr/bin/env python3
"""
Homebrew Formula Update Automation Script for gworkspace-mcp

Automates the process of updating the Homebrew formula for gworkspace-mcp:
1. Fetches latest version and SHA256 from PyPI
2. Clones/updates the homebrew-gworkspace-mcp tap repository
...
Usage:
    ...
    ./scripts/update_homebrew.py --dry-run

    # Update to latest version and push
    ./scripts/update_homebrew.py

    # Update to specific version
    ./scripts/update_homebrew.py --version 0.1.26

    # Specify custom tap repo path
    ./scripts/update_homebrew.py --tap-repo-path /path/to/tap
...
class HomebrewFormulaUpdater:
    """Manages updates to Homebrew formula for gworkspace-mcp."""
    ...
    def __init__(
        ...
        self.formula_name = "gworkspace-mcp.rb"
    ...
    def fetch_pypi_info(self, version: str | None = None) -> PackageInfo:
        ...
        if version:
            url = f"https://pypi.org/pypi/gworkspace-mcp/{version}/json"
        else:
            url = "https://pypi.org/pypi/gworkspace-mcp/json"

        try:
            self.log(f"Requesting: {url}", "debug")

            request = Request(url)
            request.add_header("User-Agent", "gworkspace-mcp-formula-updater/1.0")
    ...
    def update_formula(self, package_info: PackageInfo) -> Path:
        ...
        if version_match:
            ...
            # Extract version from URL (e.g., gworkspace_mcp-0.1.26.tar.gz)
            url_match = re.search(r"gworkspace_mcp-([0-9]+\.[0-9]+\.[0-9]+)\.tar\.gz", content)
            if url_match:
        ...
        if version_match:
            content = re.sub(r'version\s+"[^"]+"', f'version "{package_info.version}"', content)

        # Update sha256
        content = re.sub(r'sha256\s+"[^"]+"', f'sha256 "{package_info.sha256}"', content)

        # Update URL (this also updates version implicitly for URL-derived versions)
        content = re.sub(
            r'url\s+"https://files\.pythonhosted\.org/packages/[^"]+/gworkspace[_-]mcp-[^"]+\.tar\.gz"',
    ...
    def run(self, version: str | None = None) -> None:
        ...
        try:
            # Header
            print(f"\n{BOLD}{BLUE}{'=' * 60}{RESET}")
            print(f"{BOLD}{BLUE}Homebrew Formula Updater for gworkspace-mcp{RESET}")
            ...
            if not self.dry_run:
                self.log(
                    "Users can now install with: brew install masapasa/gworkspace-mcp/gworkspace-mcp",
                    "info",
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Update Homebrew formula for gworkspace-mcp",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s --dry-run                    # Test update to latest version
  %(prog)s                              # Update to latest version
  %(prog)s --version 0.1.26             # Update to specific version
...
Exit Codes:
  ...
  5: Authentication error
    ...
    parser.add_argument(
        ...
        type=Path,
        metavar="PATH",
        help="Local path to tap repository (default: ~/.homebrew_tap_update/homebrew-gworkspace-mcp)",
    )
